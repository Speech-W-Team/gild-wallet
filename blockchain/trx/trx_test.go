...
func TestTronWalletManager_GenerateWallet(t *testing.T) {
	manager := TronWalletManager{}
	_, _, err := manager.GenerateWallet()
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestTronWalletManager_GenerateAddress(t *testing.T) {
	...
	address, err := manager.GenerateAddressFromString(publicBIP44Key)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTronWalletManager_RestoreWallet(t *testing.T) {
	...
	wallet, err := manager.RestoreWalletFromString(hex.EncodeToString(privateKey.ToECDSA().D.Bytes()))
	if err != nil {
		t.Fatal(err)
	}
...
func TestTronWalletManager_RestoreWalletFromMnemonic(t *testing.T) {
	manager := TronWalletManager{}
	wallet, err := manager.RestoreWalletFromMnemonic(mnemonicTest, "")
	if err != nil {
		t.Fatal(err)
	}
